...
- 데이터 패칭 과정중 `loading`, `error` 화면은 공용 컴포넌트 `Loading.tsx`, `Error.tsx` 로 구현
  
| 로딩화면       | 에러화면     |  
|  :------------:  |  :------------: | 
| <img width="198" height="152" alt="스크린샷 2026-01-15 오후 4 07 37" src="https://github.com/user-attachments/assets/0013fac5-a415-40b3-b8e7-5753892b088c" /> | <img width="197" height="131" alt="스크린샷 2026-01-15 오후 4 07 41" src="https://github.com/user-attachments/assets/64bf875e-db36-4fd2-92a1-bce5d59daa9e" /> | 

- AI 분석 모달의 로딩 / 에러 화면은 STEP으로 분리하여 `_components/`에 구현했습니다.
...
## 😅 아쉬운 점 / 개선하고 싶은 부분
### API 호출 커스텀 훅의 공통화/추상화
이전의 프로젝트들에서는 API 주소마다 응답 형태가 동일하여 하나의 공용 커스텀훅을 제작하여 사용했습니다.<br/>
다만 이번 과제에서는 `api의 응답 형태가 모두 달랐고`,<br/>
하나의 공용 get 요청 커스텀 훅을 제작하기보다 `도메인별로 나누어서` 구현하는 방식이 깔끔하다는 판단 하에 후자의 방식을 선택했습니다.
이런 상황에서 재사용 가능한 하나의 훅을 제작해 사용 할 수 있는 방법이 무엇인지 더 탐구해서 개선하고싶습니다.

### AI 분석 결과 타입 UI 제작
AI 분석 응답중, 타입 컬럼에서 어떤 응답 값이 오는지 명확하지 않아 <br/>
`risk_assessment` 처럼 응답 값을 그대로 노출한 부분이 아쉬웠습니다. <br/>
만약 값이 지정되어있다면 값 별로 UI 를 제작하여 조금 더 사용자 친화적으로 만들 수 있을것 같습니다.

## 🚀 (선택) 시간이 더 있었다면 추가하고 싶은 기능
- 고객 리스트 조회에서 컬럼별 오름차순 내림차순 정렬 (피그마에는 반영되어있습니다.)
